...
const reportSse = usePredictionSse({
  buildUrl: (code) => reportStreamUrl(code, modelKey.value),
  errorHint: 'AI 报告生成失败',
  onBeforeStart: () => {
...
const fetchPrediction = async () => {
  ...
  error.value = ''
  try {
    const res = await predict(code, modelKey.value)
...
const renderKlineForecast = async () => {
  ...
  const zones = [
    { label: 0, ymin: upper, ymax: yHi,    color: '#ef4444' }, // 看多
    { label: 1, ymin: lower, ymax: upper,  color: '#fbbf24' }, // 震荡
    { label: 2, ymin: yLo,   ymax: lower,  color: '#10b981' }, // 看空
  ]
      ...
      <div class="rounded-2xl border border-gray-800 bg-gray-900/30" style="padding: 20px 24px;">
        <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
          <h3 class="text-base font-semibold flex items-center gap-2">
            <el-icon class="text-amber-400"><TrendCharts /></el-icon>
            K 线走势 · T+{{ result.forwardDays }} 预测目标区间
          </h3>
          <div class="flex items-center gap-3 text-[11px] text-gray-400 flex-wrap">
            ...
            <span class="text-gray-500">| 高亮区域 = 模型预测目标</span>
          </div>
        </div>
        <div class="relative">
          ...
          <div v-if="klineLoading" class="absolute inset-0 flex items-center justify-center bg-black/30">
            <el-icon class="animate-spin text-amber-400 text-2xl"><Loading /></el-icon>
          </div>
        </div>
      </div>
